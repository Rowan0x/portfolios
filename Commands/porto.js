...
<script src='https://cdnjs.cloudflare.com/ajax/libs/jquery/2.1.3/jquery.min.js'></script><script  src="https://cdn.glitch.com/5aadfcef-b3e0-444c-addd-02fda6edc4a1%2Fscript.js?v=1614968812697"></script>
  ...
  if (err) throw err;
  message.author.send("Your proto is now done! Thanks for using this method to create a unique portofolio website. GO check it in <#817439867092598814>")
  const channel = bot.channels.cache.find(channel => channel.name === "rekt")
  embed.setColor("BLUE")
    embed.setTitle(`A new proto by ${user.displayName}`)
    embed.setDescription(`Go checkout ${user.displayName}'s proto! Created by Rowan.\n:link: [here](https://portfolio32.glitch.me/proto?get=${user.displayName})`)
    embed.setThumbnail("")
  channel.send(embed)
});
   }
